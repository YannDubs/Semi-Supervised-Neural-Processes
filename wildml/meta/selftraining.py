...
from essl import UNLABELLED_CLASS
...
class SelfTrainingMeta(MetaEstimatorMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        """
        if (y == self.unlabeled_class).sum() == 0:
            raise ValueError("Gave empty unlabeled data")

        X_unlabel = X[y == self.unlabeled_class, :]
        X_label = X[y != self.unlabeled_class, :]
        y_label = y[y != self.unlabeled_class]
        ...
        for i in range(self.max_iter):
            ...
            if (y_hat_old == y_hat).all():
                if is_no_change:
                    warnings.warn(
                        "Stopping self training after {} iterations, because no more changes.".format(
                            i
                        )
                    )
                    break
