...
class SSLBatchSampler(Sampler):
    ...
    perc_lab : float or int, optional
        Percentage or number of labeled data in the batch. 
    """

    def __init__(self, targets, batch_size, perc_lab=0.5):
        self.targets = to_numpy(targets)
        self.batch_size_lab = int(batch_size * ratio_to_int(perc_lab, batch_size))
    ...
    def __iter__(self):
        ...
        idcs_lab = resample(
            idcs_lab, n_samples=len(idcs_unlab), stratify=targets[idcs_lab], replace=True
        )

        for i in range(0, len(idcs_unlab), self.batch_size_unlab):
            yield np.hstack(
                [
                    new_idcs[idcs_unlab[i : i + self.batch_size_unlab]],
...
def get_ssl_iterator(dataset, batch_size=128, **kwargs):
    ...
    try:
        ...
        y = dataset.y
    return DataLoader(dataset, batch_sampler=SSLBatchSampler(y, batch_size=batch_size), **kwargs)
...
def get_supervised_iterator(dataset, **kwargs):
    ...
    return DataLoader(dataset, sampler=SupervisedSampler(y), **kwargs)
