...
def get_Attender(attention, **kwargs):
    """
    Get attention module that matches key and query to compute attention along `dim=1`.
    ...
    if not isinstance(attention, str):
        return partial(attention, **kwargs)

    attention = attention.lower()
    if attention == "multiplicative":
        Attender = partial(MultiplicativeAttender, **kwargs)
    elif attention == "additive":
        Attender = partial(AdditiveAttender, **kwargs)
    elif attention == "scaledot":
        Attender = partial(DotAttender, is_scale=True, **kwargs)
    elif attention == "cosine":
        Attender = partial(CosineAttender, **kwargs)
    elif attention == "manhattan":
        Attender = partial(DistanceAttender, p=1, **kwargs)
    elif attention == "euclidean":
        Attender = partial(DistanceAttender, p=2, **kwargs)
    elif attention == "weighted_dist":
        Attender = partial(DistanceAttender, p=1, is_weight=True, **kwargs)
    elif attention == "multihead":
        Attender = partial(MultiheadAttender, **kwargs)
    elif attention == "transformer":
        Attender = partial(TransformerAttender, **kwargs)
    else:
...
class MultiheadAttender(nn.Module):
    ...
    def __init__(
        ...
        self.out_size = out_size
        self.post_processor = (
            nn.Linear(value_size, out_size) if is_post_process or value_size != out_size else None
        )
...
class TransformerAttender(MultiheadAttender):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mlp = MLP(
            self.out_size, self.out_size, hidden_size=self.out_size, activation=nn.ReLU()
        )
