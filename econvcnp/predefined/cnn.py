...
from essl.utils.helpers import make_depth_sep_conv, channels_to_2nd_dim, channels_to_last_dim
        ...
        **kwargs,
        ...
        if isinstance(tmp_channels, int):
            if is_force_hid_smaller and tmp_channels > max(in_channels, out_channels):
                ...
                warnings.warn(
                    txt.format(tmp_channels, out_channels, in_channels, new_tmp_channels)
                )
...
class UnetCNN(CNN):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            in_channels, out_channels, tmp_channels=tmp_channels, ConvBlock=ConvBlock, **kwargs
        )
    ...
    def _get_in_out_channels(self, in_channels, out_channels, tmp_channels, n_blocks):
        ...
        channel_list = [
            round(self.factor_chan ** i * tmp_channels) for i in range(n_tmp_blocks // 2 + 1)
        ]
        ...
        if not self.is_skip_resize:
            ...
            in_out_channels = (
                [(in_channels, tmp_channels)] + in_out_channels + [(tmp_channels, out_channels)]
            )
